...
export default function DraftsView({ initialId }: { initialId?: string }) {
  ...
  useEffect(() => {
    fetch('/api/admin/drafts').then(r => r.json()).then(async (d: Draft[]) => {
      setDrafts(d)
      if (initialId) {
        const match = d.find(x => x.id === initialId)
        if (match) {
          selectDraft(match)
        } else {
          // Draft may already have been approved/rejected, or not exist at all —
          // fall back to fetching it directly so a stale Discord link still shows something.
          const res = await fetch(`/api/admin/drafts/${initialId}`)
          const single = res.ok ? await res.json() : null
  ...
  if (notFound) {
    return (
      <div style={{ padding: 40, color: '#9a7d5a', fontSize: 14 }}>
        That draft couldn&rsquo;t be found — it may have been deleted.
      </div>
    )
  }
